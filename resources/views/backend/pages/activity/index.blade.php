      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div class="bg-white shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
              <div class="mb-10">
                <a href="{{ route('admin.kegiatan.create') }}" class="button">+ Tambah</a>
              </div>
                ...
                <tbody class="bg-white dark:bg-slate-800">
                  ...
                  <tr>
                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-900">
                      {{ (($activities->perPage() * $activities->currentPage()) - $activities->perPage()) + ($loop->index + 1) }}
                    </td>
                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-900">
                      <img src="{{ asset('storage/uploads/'.$activity->thumbnail_url) }}" width="100" height="100" alt="Ketua Pendahulu">
                    </td>
                    ...
                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 pr-8 text-slate-500 dark:text-slate-900">
                      <x-dropdown align="right" width="48">
                        ...
                        <x-slot name="content">
                          <x-dropdown-link :href="route('admin.kegiatan.edit', $activity->id)">
                            {{ __('Edit') }}
                          </x-dropdown-link>
                          <form
                            action="{{ route('admin.kegiatan.destroy', $activity->id) }}"
                            method="POST"
                          >
                            @csrf
                            @method('DELETE')
